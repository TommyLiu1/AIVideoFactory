...
@click.option('--valid_duration', prompt='有效时间(天数)', type=int, help='用户有效期天数')
def create_db_user(username, password, user_type, valid_duration):
    """创建数据库用户"""
    pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")
    salt = secrets.token_hex(16)  # 生成随机盐
    hashed_password = pwd_context.hash(password + salt)
    valid_from = datetime.datetime.now()
    valid_to = valid_from + datetime.timedelta(days=valid_duration)
    user = UserDBService.create_user(
        username=username,
